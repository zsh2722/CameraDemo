...
public class Camera2Helper {
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            mBackgroundHandler.post(new Camera2Helper.ImageSaver(reader.acquireNextImage(), mFile, listener));
        }
    };
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback = new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull TotalCaptureResult result) {
                    ...
                    Log.d("Camera2Helper", "onCaptureCompleted" + "保存照片成功");
                }
            };
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                Log.d("Camera2Helper", "maxPreviewWidth:" + maxPreviewWidth);
                Log.d("Camera2Helper", "maxPreviewHeight:" + maxPreviewHeight);
...
//                mPreviewSize = determineBestSize(map.getOutputSizes(SurfaceTexture.class), rotatedPreviewWidth, rotatedPreviewHeight);
                Log.d("Camera2Helper", "TextureView:" + width + "*" + height);
                Log.d("Camera2Helper", "mPreviewSize:" + mPreviewSize.getWidth() + "*" + mPreviewSize.getHeight());
                float i = mPreviewSize.getWidth() / (float) mPreviewSize.getHeight();
                Log.d("Camera2Helper", "i:" + i);
                Log.d("Camera2Helper", "width*i:" + (width * i));
    ...
    private Size determineBestSize(Size[] choices, int textureViewWidth, int textureViewHeight) {
        Size bestSize = null;
        Size size;
        int numOfSizes = choices.length;
        for (int i = 0; i < numOfSizes; i++) {
            size = choices[i];
            boolean isDesireRatio = (size.getWidth() / 4) == (size.getHeight() / 3);
            boolean isBetterSize = (bestSize == null) || size.getWidth() > bestSize.getWidth();
    ...
    private static class ImageSaver implements Runnable {
        ...
        private AfterDoListener mDoListener;
        ...
        public ImageSaver(Image image, File file, AfterDoListener listener) {
            mDoListener = listener;
            mImage = image;
            mFile = file;
        }
        ...
        public void run() {
            ...
            try {
                ...
                output.write(bytes);//写到文件输出流
                output.flush();
                output.close();
                mDoListener.onAfterTakePicture(mFile.getAbsolutePath());
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
                mImage.close();
    ...
    public interface AfterDoListener {
        ...
        void onAfterTakePicture(String path);
    }
}
